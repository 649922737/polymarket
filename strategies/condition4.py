    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if data and len(data) > 0:
                # 聚合所有返回的 Candle (防止返回的是多个 1分钟 Candle)
                lows = [float(x[1]) for x in data]
                highs = [float(x[2]) for x in data]
                min_low = min(lows)
                max_high = max(highs)
                fluc = max_high - min_low

                # Logging for debug
                candle_ts = data[0][0]
                candle_time = datetime.fromtimestamp(candle_ts, timezone.utc).strftime('%H:%M:%S')
                logger.info(f"Condition4: 获取到上个周期波动: {fluc:.2f} (Req: {prev_start.strftime('%H:%M')} - {prev_end.strftime('%H:%M')}, Got: {candle_time}, Count: {len(data)})")
                return fluc
            else:
                logger.warning(f"Condition4: 上个周期无 Candle 数据 ({start_str})")
        else:
            logger.warning(f"Condition4: Coinbase API Error: {resp.status_code} {resp.text}")
    except Exception as e:
        logger.error(f"Condition4: 获取历史数据失败: {e}")
...
def check(state, config, indicators):
    ...
    if current_abs_change > ratio_threshold and current_abs_change > abs_limit:
        ...
        return {
            "action": "trade",
            "side": side,
            "reason": f"Condition_4_FLUC_BREAK ({current_abs_change:.2f} > {ratio_pct}*{prev_fluc:.2f} & > {abs_limit})"
        }
